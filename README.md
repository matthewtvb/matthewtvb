# Hello, I'm Matthew! 👋


## 👤 About Me

Cybersecurity Engineering student at George Mason University with hands-on experience in security operations, cloud security, and risk management. I've built SIEM detection labs using Splunk and Wazuh, identified misconfigurations in AWS environments, and conducted risk assessments using NIST CSF 2.0 and NIST RMF.

## 👨‍💻 Cybersecurity Projects
A collection of practical lab exercises and more performed in cybersecurity and systems programming.

| Project Topic | Description | Tools Used |
|-----------|-------------|------------|
| [Advanced Port Scanning](https://github.com/matthewtvb/cybersecurity-labs/blob/9ee170006548ee31656f17df6f4b12f6bb3b83fc/Advanced%20Port%20Scanning.pdf) | Combining Metasploit and Nmap for reconnaissance | Metasploit, Nmap, PostgreSQL |
| [Wazuh SIEM & File Integrity Monitoring](https://github.com/matthewtvb/cybersecurity-labs/blob/8d02f33523326e8caa6ad3200d890f2babb32132/WAZUH%20Home%20Lab.pdf) | Deployed Wazuh SIEM on an Ubuntu VM to monitor a Windows host with real-time FIM alerts and simulated brute force attacks mapped to MITRE ATT&CK | Wazuh, Ubuntu, Kali Linux, Hydra, Nmap, VirtualBox |
| [Wireshark & Network Traffic](https://github.com/matthewtvb/cybersecurity-labs/blob/9ee170006548ee31656f17df6f4b12f6bb3b83fc/Basics%20of%20Wireshark%20and%20Network%20Traffic.pdf) | Capturing and analyzing live and saved packets | Wireshark, Kali Linux |
| [Breaking Ciphers & Hashes](https://github.com/matthewtvb/cybersecurity-labs/blob/main/Breaking%20Ciphers%20and%20Hashes%20Cipher%20and%20MD5%20Collision.pdf) | Substitution cipher cracking and MD5 collision attacks | Python, GCC, md5collgen |
| [GRUB Bootloader Security](https://github.com/matthewtvb/cybersecurity-labs/blob/main/GRUB%20Bootloader%20Security%20Configuration%20and%20User%20Access%20Control.pdf) | Configuring bootloader security and user access | Ubuntu, GRUB, gedit |
| [IP/MAC Spoofing & ARP Poisoning](https://github.com/matthewtvb/cybersecurity-labs/blob/main/IP%20Spoofing%2C%20MAC%20Spoofing%2C%20and%20ARP%20Poisoning.pdf) | Performing MITM attacks using ARP spoofing | Kali Linux, Wireshark, arpspoof |
| [Network Reconnaissance](https://github.com/matthewtvb/cybersecurity-labs/blob/main/Network%20Reconnaissance%20and%20Service%20Enumeration%20Using%20Kali%20Linux.pdf) | Host discovery, service enumeration, and vulnerability scanning | Nmap, Netcat, DirBuster |
| [Public-Key Cryptography (RSA)](https://github.com/matthewtvb/cybersecurity-labs/blob/main/Public-Key%20Cryptography%20with%20RSA.pdf) | Implementing RSA encryption, decryption, and digital signatures | C, OpenSSL, GCC |
| [Race Condition Exploitation](https://github.com/matthewtvb/cybersecurity-labs/blob/main/Race%20Condition%20Exploitation%20via%20TOCTOU%20Vulnerability.pdf) | Exploiting TOCTOU vulnerabilities in Set-UID programs | C, GCC, Linux system calls |
| [Secret-Key Encryption Modes](https://github.com/matthewtvb/cybersecurity-labs/blob/main/Secret-Key%20Encryption%20Modes%2C%20Padding%2C%20and%20Vulnerabilities.pdf) | Exploring ECB, CBC modes and IV reuse vulnerabilities | OpenSSL, Python |
| [Stack-Based Buffer Overflow](https://github.com/matthewtvb/cybersecurity-labs/blob/main/Stack-Based%20Buffer%20Overflow%20Attack.pdf) | Exploiting a vulnerable C program to gain root shell | GCC, GDB, Python |
| [Stored Cross-Site Scripting (XSS)](https://github.com/matthewtvb/cybersecurity-labs/blob/main/Stored%20Cross-Site%20Scripting.pdf) | Exploiting XSS vulnerabilities in DVWA | DVWA, BeEF, Browser |
| [Thread Synchronization](https://github.com/matthewtvb/cybersecurity-labs/blob/main/Thread%20Synchronization%20Lab%20Mutexes%20and%20Race%20Conditions.pdf) | Using mutexes to prevent race conditions in C | C, pthread, GCC |
| [USB Partitioning & Memory Analysis](https://github.com/matthewtvb/cybersecurity-labs/blob/main/USB%20Drive%20Partitioning%20%26%20Memory%20Layout%20Analysis.pdf) | Partitioning USB drives and analyzing program memory layout | fdisk, mkfs, GDB, objdump |
| [Linux Command Line Practice](https://github.com/matthewtvb/cybersecurity-labs/blob/main/Linux%20Command%20Line%20starting%20point.pdf) | Basic Linux file and directory operations | Bash, Terminal |
| [Sony MOVEit Breach Risk Assessment](https://github.com/matthewtvb/cybersecurity-Other-Projects/blob/30f9bab5d6bcaca96088dc5740f6e2988328afc4/Sony%20Risk%20Assessment%20.pdf) | Preformed a risk assessment on SONY's 2023 Breach | Word |
| [Risk Register](https://github.com/matthewtvb/cybersecurity-Other-Projects/blob/30f9bab5d6bcaca96088dc5740f6e2988328afc4/Sony%20Risk%20Register.xlsx) | Preformed a risk register on SONY's 2023 Breach | Excel |
| [AWS Cloud Misconfiguration Detection](https://github.com/matthewtvb/cybersecurity-labs/blob/0b2b5075c187d66d9df0be782a1a79019510c441/AWS%20Cloud%20Misconfiguration%20Detection.pdf) | Simulated and detected two real-world AWS misconfigurations, a publicly exposed S3 bucket and an over-privileged IAM user using CloudTrail and GuardDuty | AWS S3, IAM, CloudTrail, GuardDuty |
| [Splunk SIEM Home Lab](https://github.com/matthewtvb/cybersecurity-labs/blob/0b2b5075c187d66d9df0be782a1a79019510c441/splunk%20lab.pdf)|Installed Splunk Enterprise, loaded the BOTS v3 attack dataset, ran four SPL investigation queries to detect brute force, network scanning, web attacks, and suspicious process execution, then built a two-panel SOC dashboard|Splunk, SPL, BOTS v3, Windows, MITRE ATT&CK|
|[Change Healthcare NIST CSF 2.0 Gap Analysis](https://github.com/matthewtvb/cybersecurity-Other-Projects/blob/6335ec510298229e4c3a506e6049008c8b3a4f86/Change%20Healthcare%20NIST%20CSF.pdf)|Conducted a gap analysis of the 2024 Change Healthcare ransomware breach across all six NIST CSF 2.0 functions, identified critical control failures, and produced prioritized remediation recommendations|NIST CSF 2.0, Word|
| [Airline Delay Analysis](https://github.com/matthewtvb/cybersecurity-labs/blob/4ce7a58d4c611494e607d03af0764c3f034ea350/airline-data-analysis/airline_delay_analysis.py) | Loaded and cleaned 1,800+ records of U.S. flight delay data from the BTS, aggregated by airline and cause to identify that carrier delays account for 56% of total delay minutes | Python, pandas, Excel |
| [Active Directory Home Lab](https://github.com/matthewtvb/cybersecurity-labs/blob/73e9d4e1d64248f1e8cd9d0d2c6f7226def2151d/Active%20Directory%20Home%20Lab.pdf) | Built a working AD environment in VirtualBox, configured AD DS, managed OUs/users/groups, applied Group Policy, and simulated IT ops workflows including onboarding, offboarding, password resets, and account unlocks | VirtualBox, Windows Server 2022, ADUC, Group Policy Management Console, PowerShell |
---
